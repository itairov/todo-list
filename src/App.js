...
function App() {
  ...
  //useEffect - для того чтоб понять , произошел ли рендер = [можно узнать какое свойство изменилось]
  // он получает то что вызывет при изменении чего-то,
  // []-означает что не нужно следить ни за кем, как только произошел рендер, вызови содержимое useEffect
  // [list] если передать зависимость(list) - то как только обновиться list, вызови содержимое
  useEffect(() => {
    axios
...
//свойства который без значиния вниз, со значением вверх
// 55min #3
